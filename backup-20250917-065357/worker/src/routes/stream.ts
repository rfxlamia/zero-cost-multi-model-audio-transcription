    ...
    start: async (controller) => {
      ...
      const enqueueHeartbeat = () => {
        ...
        heartbeatTimer = setTimeout(() => {
          controller.enqueue(
            encoder.encode(toEvent('heartbeat', { jobId: id, ts: new Date().toISOString() }))
          )
          enqueueHeartbeat()
        }, 15000)
      }
      ...
      try {
        ...
        const persistState = async (force = false) => {
          ...
          if (!force) {
            ...
            if (dirtyCount < WRITE_BATCH_THRESHOLD && elapsed < WRITE_INTERVAL_MS) return
          }
        ...
        const enhancedBatch: Promise<void>[] = []
        let enhancedDone = 0
        for (let i = 0; i < job.chunks.length; i++) {
          const ch = job.chunks[i]
        ...
        send('sse-error', { message: (e as any)?.message || 'SSE error' })
      } finally {
        if (heartbeatTimer) {

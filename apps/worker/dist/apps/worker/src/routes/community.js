...
community.post('/api/community/submit', async (c) => {
    try {
        ...
        const requestToken = (typeof submission.turnstileToken === 'string' && submission.turnstileToken) ||
...
community.post('/api/community/upvote', async (c) => {
    try {
        ...
        const requestToken = (typeof request.turnstileToken === 'string' && request.turnstileToken) ||
            c.req.header('cf-turnstile-token') ||
            undefined;
        if (c.env.TURNSTILE_SECRET) {
            if (!requestToken) {

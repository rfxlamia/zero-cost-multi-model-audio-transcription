...
import { CHUNK_DURATION_SECONDS, MAX_AUDIO_DURATION_MINUTES, MAX_FILE_SIZE_MB, } from '@transcriptorai/shared/constants';
import { verifyTurnstileToken } from '../utils/turnstile';
import { logAnomaly } from '../utils/logging';
...
transcribe.post('/api/transcribe/start', async (c) => {
    ...
    const requestToken = (typeof payload?.turnstileToken === 'string' && payload.turnstileToken) ||
    ...
    const sizeBytes = typeof payload?.sizeBytes === 'number' && payload.sizeBytes > 0 ? payload.sizeBytes : null;
...
transcribe.post('/api/transcribe/:id/chunk', async (c) => {
    ...
    const duplicate = job.chunks.some((existing) => {
        if (!existing || typeof existing.audioHash !== 'string')
            return false;
        return existing.audioHash === body.audioHash && existing.index !== nextIndex;
    });
    if (duplicate) {
        logAnomaly('duplicate_audio_hash', {
            jobId: id,

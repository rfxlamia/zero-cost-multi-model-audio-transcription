...
import { EXPORT_DEFAULTS, getExportSegments, segmentsToSrt, segmentsToTxt, segmentsToVtt, } from '@transcriptorai/shared/utils';
export const exp = new Hono();
const SUPPORTED_FORMATS = new Set(['txt', 'srt', 'vtt', 'json']);
const buildAttachmentHeaders = (filename, contentType) => ({
    'Content-Type': contentType,
    'Content-Disposition': `attachment; filename="${filename}"`,
});
const deriveSegments = (job) => getExportSegments(job, {
...
exp.get('/api/export/:file', async (c) => {
    ...
    if (!segments.length)
        return c.json({ error: 'No chunks to export' }, 400);

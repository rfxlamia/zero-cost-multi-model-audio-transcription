...
function checkRateLimit(ip, windowMs = 1000, max = 10) {
    const now = Date.now();
...
metrics.get('/api/metrics', async (c) => {
    ...
    for (const p of providers) {
        ...
        const nearLimit = await preemptiveSwitch({ QUOTA_COUNTERS: c.env.QUOTA_COUNTERS }, p);
        const enabled = (p === 'groq' &&
            !!c.env.GROQ_API_KEY &&
    ...
    const semaphores = {
        providerConcurrency: providerSemaphore?.limit ?? 5,
        kvConcurrency: kvSemaphore?.limit ?? 5,
    };

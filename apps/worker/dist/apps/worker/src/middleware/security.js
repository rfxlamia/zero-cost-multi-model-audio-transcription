...
const REQUIRED_SECRETS = ['APP_SECRET'];
const OPTIONAL_SECRETS = ['GROQ_API_KEY', 'HF_API_TOKEN', 'TURNSTILE_SECRET'];
...
function parseOriginList(originList) {
    if (!originList)
        return [];
    return originList
        .split(',')
...
function applySecurityHeaders(c) {
    ...
    const originEntries = new Set(["'self'"]);
...
async function verifySignature(secret, value, signature) {
    try {
        ...
        console.warn('[security] signature verification error', error.message);
        return false;
    }
}
function isSecureRequest(url, forwardedProto) {
    if (forwardedProto) {
...
async function applyRateLimits(env, configs) {
    if (!configs.length)
    ...
    for (const cfg of configs) {
        ...
        if (nextCount > cfg.max) {
            ...
            return { allowed: false, meta: [meta] };
        }
    ...
    for (const state of states) {
        ...
        await env.QUOTA_COUNTERS.put(state.key, JSON.stringify({
            scope: state.scope,
    ...
    return { allowed: true, meta: metas };
}
async function ensureAnonId(c) {
...
export const securityMiddleware = async (c, next) => {
    ...
    const ipHeader = c.req.header('cf-connecting-ip') || c.req.header('x-forwarded-for')?.split(',')[0]?.trim();
